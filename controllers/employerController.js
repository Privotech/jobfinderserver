...
async function updateCompany(req, res, next) {
  try {
    const company = await Company.findOneAndUpdate({ owner: req.user._id }, req.body, { new: true });
    if (!company) return res.status(404).json({ success: false, error: 'Company not found' });
    res.json({ success: true, data: company });
  } catch (err) {
    next(err);
...
async function getJobs(req, res, next) {
  try {
    ...
    const jobs = await Job.find({ company: company._id }).populate('company').sort({ createdAt: -1 }).lean();
...
async function createJob(req, res, next) {
  try {
    const company = await Company.findOne({ owner: req.user._id });
    if (!company) return res.status(400).json({ success: false, error: 'Create company first' });
    const job = await Job.create({ ...req.body, company: company._id, creator: req.user._id });
...
async function updateJob(req, res, next) {
  try {
    const company = await Company.findOne({ owner: req.user._id });
    if (!company) return res.status(404).json({ success: false, error: 'Company not found' });
    const job = await Job.findOneAndUpdate({ _id: req.params.id, company: company._id }, req.body, { new: true });
...
async function getApplications(req, res, next) {
  try {
    ...
    if (!job) return res.status(404).json({ success: false, error: 'Job not found' });
    const apps = await Application.find({ job: job._id }).populate('candidate').populate('resume').sort({ create